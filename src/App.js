...
const App = () => {
  ...
  const handleUnitsClick = (e) => {
    ...
    const isCelsium = currentUnit === 'C';
    button.innerText = isCelsium ? '°F' : '°C';
    setUnits(isCelsium ? 'metric' : 'imperial');
    console.log(units);
  };

  const enterKeyPressed = (e) => {
    if (e.keyCode === 13) {
      setCity(e.currentTarget.value ? e.currentTarget.value : 'Cleveland');
